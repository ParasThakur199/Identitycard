...
public class LabelServiceImpl implements LabelService{
    ...
    public String saveLabelsFromBlocks(FullSubmitRequest request) {
        for (BlockLabelRequest blockLabel : request.getBlockLabelData()) {
            String block = blockLabel.getBlock();
	...
	@Override
	public List<LabelGetDTO> getAllLabels() {
		List<LabelEntity> entities = labelRepository.findAll();
		
		// Convert Entity list to DTO list
				List<LabelGetDTO> dtoList = entities.stream()
				        .map(entity -> {
				        	LabelGetDTO dto = new LabelGetDTO(); 
